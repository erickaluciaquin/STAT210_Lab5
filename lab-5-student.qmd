...
teacher_evals %>%
  select(teacher_id) %>%
  nrow() == nrow(teacher_evals)
```

A single row in a data set is identified by the teacher ID. 
...
teacher_evals_clean <- teacher_evals %>%
  ...
  mutate(teacher_id = as.character(teacher_id), # changes data type to chr variables
...
# code chunk for Q5
uniqueinstructor <- n_distinct(teacher_evals_clean$teacher_id)
uniquecourse <- n_distinct(teacher_evals_clean$course_id)
uniqueinstructor
uniquecourse
```
